...
GLvoid drawScene(GLvoid) {
	...
	GLfloat in_fog_color[4] = { fog_color, fog_color, fog_color, 1.0 };
	GLfloat in_density = density;
	GLfloat in_start = start;
	GLfloat in_end = end;

	glEnable(GL_FOG);

	if(fog == LINEAR)	glFogf(GL_FOG_MODE, GL_LINEAR);
	else if(fog == EXP)	glFogf(GL_FOG_MODE, GL_EXP);
	else if (fog == EXP2)	glFogf(GL_FOG_MODE, GL_EXP2);

	glFogfv(GL_FOG_COLOR, in_fog_color);
	glFogf(GL_FOG_START, in_start);
	glFogf(GL_FOG_END, in_end);

	glFogf(GL_FOG_DENSITY, in_density);
...
void Keyboard(unsigned char key, int x, int y) {
	...
	case '2':
		...
		break;
	case '3':
		if (0 < density)	density -= 0.05f;
		break;
	case '4':
		if (1 > density)	density += 0.05f;
		break;
	case '5':
		start -= 10.f;
		break;
