...
void Draw_shape2(bool shape, float x, float y);
...
void Draw_shape2(bool shape, float x, float y) {
	...
	glBegin(GL_POLYGON);
	if (shape == triangle) {
		glVertex2f(0, 0 + shape_size);
		glVertex2f(0 - (shape_size*1.414), 0 - (shape_size / 2));
		glVertex2f(0 + (shape_size*1.414), 0 - (shape_size / 2));
	}

	else if (shape == rectangle) {
		glVertex2f(0 - shape_size, 0 - shape_size);
		glVertex2f(0 - shape_size, 0 + shape_size);
		glVertex2f(0 + shape_size, 0 + shape_size);
		glVertex2f(0 + shape_size, 0 - shape_size);
	}
