...
public:
	float x = 0, z = 0;
	bool radian_x_L = false;
	bool radian_y_L = false;
	bool radian_z_L = false;

	bool radian_x_R= false;
	bool radian_y_R = false;
	bool radian_z_R = false;
...
void Keyboard(unsigned char key, int x, int y) {
	...
	case 'u':  case 'U':
		if (circle.z > -250.f) circle.z -= 10.f;
		circle.radian_x_L = true;
		break;

	case 'j':  case 'J':
		if (circle.z < 250.f) circle.z += 10.f;
		circle.radian_x_R = true;
		break;

	case 'h':  case 'H':
		if (circle.x > -250.f) circle.x -= 10.f;
		circle.radian_z_L = true;
		break;

	case 'k':  case 'K':
		if (circle.x < 250.f) circle.x += 10.f;
		circle.radian_z_R = true;
		break;
	}
	glutPostRedisplay();
}
