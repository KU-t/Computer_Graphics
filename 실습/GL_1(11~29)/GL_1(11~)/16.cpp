...
void Keyboard(unsigned char key, int x, int y) {
	...
	case 'z':	case 'Z':
		...
		break;
	case 'l': case 'L':
		obj.left_obj_sw = (obj.left_obj_sw + 1) % 2;
		break;
	case 'r': case 'R':
		obj.right_obj_sw = (obj.right_obj_sw + 1) % 2;
		break;
	case 'o': case 'O':
...
void Change_Angle_xyz() {
	...
	if (obj.left_obj_sw) obj.left_obj_radian += 2;
	if (obj.right_obj_sw) obj.right_obj_radian += 2;
	glutPostRedisplay();
}
