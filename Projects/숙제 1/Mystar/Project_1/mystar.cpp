...
public:
	...
	void Draw() {
		...
		for (int i = 0; i < 360; i += 60) {
			glPushMatrix();
			glRotated(i, 0, 0, 1);
			glScalef(size / 7, size / 3, size / 5);
			glColor4f(R, G, B, 0.f);
			glutSolidCone(10, 10, 10, 10);
			glColor4f(1.f, 1.f, 1.f, 0.f);
			glutWireCone(11, 11, 5, 5);
			glPopMatrix();
...
public:
	...
	float tx, ty;
	TRIANGLE_TYPE type;
	int cnt_animation = 60;
	bool sw_animation = false;

	void Draw() {
		if (exist && cnt_animation == 60) {
			glPushMatrix();
		...
		if (sw_animation) {
			...
			if (cnt_animation == 60) {
				...
				for (int i = 0; i < MAX_STARS; i++) {
					if (stars[i].exist == false) {
						stars[i].exist = true;
						stars[i].cx = (float)(rand() % 1600) - 800.f;
						stars[i].dx = stars[i].cx / 1000.f;
						stars[i].cy = (float)(rand() % 1600) - 800.f;
						stars[i].dy = stars[i].cy / 1000.f;
						stars[i].cz = (float)(rand() % 1000);
						stars[i].dz = stars[i].cz / 1000.f;
						stars[i].size = (float)(rand() % 10) + 10.f;
						stars[i].R = R;
						stars[i].G = G;
						stars[i].B = B;
						break;
	...
	void animation() {
		...
		if (sw_animation) {
			...
			for (int i = 0; i < 360; i += 60) {
				glPushMatrix();
				glRotated(i, 0, 0, 1);
				glScalef(cnt_animation / 7, cnt_animation / 3, cnt_animation / 5);
				glutSolidCone(10, 10, 10, 10);
				glPopMatrix();
			}
			glPopMatrix();
...
GLvoid drawScene(GLvoid) {
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	gluLookAt(move_x, move_y, move_z + 600.f, 0.f, 0.f, 300.f, 0.f, 1.f, 0.f);
...
void Draw_bottom() {
	glPushMatrix();
	glTranslatef(0.f, -800.f, 0.f);
	glScalef(5000.f, 1.f, 5000.f);
